...
class WildernessMap extends Map {
    ...
    WildernessMap(int size) {
        ...
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                ...
                switch (i) {
                    case 0:
                        tiles[row][col] = createTile(Tile.Type.SWAMP, row, col);
                        break;
                    case 1:
                        tiles[row][col] = createTile(Tile.Type.FOREST, row, col);
                        break;
                    case 2:
                        tiles[row][col] = createTile(Tile.Type.WATER, row, col);
                        break;
                }
            }
        }
    ...
    public Tile createTile(Tile.Type type, int row, int col) {
        switch (type) {
            case SWAMP:  return new SwampTile(row, col);
            case FOREST: return new ForestTile(row, col);
            case WATER:  return new WaterTile(row, col);
